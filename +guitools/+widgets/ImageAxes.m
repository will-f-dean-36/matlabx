...
%
    ...
    properties (SetAccess=?guitools.widgets.ImageAxesTool)
    ...
    properties (Access=?guitools.widgets.ImageAxesTool)
    ...
    properties (Access=private)
        CData_              (:,:,:) = guitools.widgets.ImageAxes.placeholderImage;
    ...
    properties (Access=?guitools.widgets.ImageAxesTool, Dependent)
        ParentFig
    ...
    methods (Access=private)
        ...
        function updateColorbar(obj)
            ...
            switch obj.CDataType
                case 'grayscale'
                    [ticks,labels] = guitools.widgets.ImageAxes.getColorbarTickLabels(obj.CDataClass,clim);
                case 'rgb'
                    if obj.ShowComposite
                        [ticks,labels] = guitools.widgets.ImageAxes.getColorbarTickLabels(obj.CDataClassStore{obj.ChannelIdx},clim);
                    end
            end
    ...
    methods (Access=?guitools.widgets.ImageAxesTool, Hidden=true)
    ...
    methods
        ...
        function tf = matches(obj, tgt, kind, ~)
            ...
            ia = ancestor(tgt,'guitools.widgets.ImageAxes');
            tf = (ia == obj);
        end
        ...
        function onKeyPress(obj, evt, tgt)
            ...
            fprintf('guitools.widgets.ImageAxes.onKeyPressed()\n');
            fprintf('Character: %s\n',evt.Character)
            fprintf('Key: %s\n',evt.Key)
    ...
    methods
        ...
        % load all tools in guitools.widgets.tools
        ...
        function loadTool(obj, name)
            ...
            obj.ToolList(char(name)) = guitools.widgets.tools.(char(name))(obj);
        end
    ...
    methods
        ...
        function set.CData(obj,cdata)
            ...
            if isempty(cdata)
                cdata = guitools.widgets.ImageAxes.placeholderImage();
            end
        ...
        function v = get.DisplayCData(obj)
            switch obj.CDataType
                case 'grayscale'
                    v = guitools.utils.rescaleLinear(obj.CData,obj.CLim);
                case 'rgb'
                    v = obj.CData_;
            end
        end
    ...
    methods (Access=private)
        ...
        function setCData(obj,cdata)
            ...
            evtData = guitools.widgets.events.CDataChangedEventData(obj.CData_,cdata);
            % update private backing
            obj.CData_ = cdata;
            % refresh view
            obj.refreshView();
            ...
            switch obj.MultiChannelType
                case 'grayscale'
                    ...
                    switch obj.ChannelColorMode
                        case 'colors'
                            clrs = cellfun(@(C) guitools.utils.getColor(C),obj.ChannelColors,'UniformOutput',false);
                            colors = cell2mat(clrs');
                            I = guitools.utils.mergeChannelsRGB_add(obj.CDataStore,clims,colors(1:obj.nChannels,:));
                        case 'luts'
                            I = guitools.utils.mergeChannelsRGB_LUT(obj.CDataStore,clims,obj.CDataMapStore);
                    end
                case 'rgb'
                    I = guitools.utils.mergeRGBImages(obj.CDataStore,"Method","additive");
            end
        end
        ...
        function updateCDataMapStore(obj)
            switch obj.ChannelColorMode
                case 'colors'
                    for c = 1:obj.nChannels
                        obj.CDataMapStore{c} = guitools.utils.colorGradient([0 0 0],guitools.utils.getColor(obj.ChannelColors{c}),256);
                    end
                case 'luts'
    ...
    methods
        ...
        function nextChannel(obj)
            obj.ChannelIdx = guitools.utils.wrapStep(obj.ChannelIdx,1,1,obj.nChannels);
        end
        ...
        function val = get.ChannelColors(obj)
            ...
            elseif numel(val)<obj.nChannels % too few colors
                ...
                avail = setdiff(guitools.widgets.ImageAxes.getColorNames,val);
        ...
        function setCLim(obj,clim,idx)
            arguments
                obj (1,1) guitools.widgets.ImageAxes
                clim (1,2) double
                idx (:,1) = []
            end
    ...
    methods (Static)
        ...
        % return names of all classes in guitools.widgets.tools
        function names = getToolClassNames()
            % get list of tool class names in guitools.widgets.tools using matlab.metadata.Namespace

            % string array
            % names = string({matlab.metadata.Namespace.fromName("guitools.widgets.tools").ClassList.Name})';

            % cell array of char vectors
            names = {matlab.metadata.Namespace.fromName("guitools.widgets.tools").ClassList.Name}';
        end

        % return names of all tool classes (just the last part)
        function names = getToolNames()

            classNames = guitools.widgets.ImageAxes.getToolClassNames();
        ...
        function ax = demo(name)
            ...
            switch name
                case 'default'
                    ax = guitools.widgets.ImageAxes(fig,...
