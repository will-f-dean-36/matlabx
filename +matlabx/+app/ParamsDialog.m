...
%             for i = 1:n
%                 spec = self.Specs{i};
%                 S = matlabx.app.ParamsDialog.parseSpec_(spec, i);
% 
%                 uilabel(self.Grid, ...
%                     'Text', char(S.Label), ...
...
%                     values.(S.Name) = self.readControlValue_(ctrl, S);
%                 end
%             catch ME
...
%                             'Parameter "%s" must be a valid number.', S.Label);
%                     end
% 
%                 case "logical"
...
%             if ~iscell(spec) || ~(numel(spec)==3 || numel(spec)==4)
%                 error('ParamsDialog:InvalidSpec', ...
%                     'Spec %d must be {name,label,type} or {name,label,type,default}.', idx);
%             end
% 
%             name = string(spec{1});
%             label = string(spec{2});
...
%             end
% 
%             validTypes = ["string","char","double","logical"];
...
%                 default = spec{4};
%             else
%                 default = matlabx.app.ParamsDialog.defaultValueForType_(type);
%             end
% 
%             S = struct( ...
%                 'Name', char(name), ...
...
%     %     'Enter parameter values', ...
%     %     {'Name','Enter your name','string'}, ...
%     %     {'Age','Enter your age','double',25, @(x)x>=0 && x<=100, ...
...
%                     'Validator failed for parameter "%s": %s', char(S.Label), ME.message);
%             end
% 
...
%             end
% 
%             validTypes = ["string","char","double","logical","choice"];
%             if ~any(type == validTypes)
%                 error('ParamsDialog:InvalidType', ...
%                     'Spec %d type must be one of: %s.', ...
%                     idx, strjoin(cellstr(validTypes), ', '));
